test_that("bam2db works with zUMIs protocol + gene features", {
  skip_on_cran()
  library(exonBlocks)
  human_bam <- test_path("..", "..", "meta", "bam", "human_pbmc_Smart-seq3xpressV3.bam")
  skip_if_not(file.exists(human_bam), "Human BAM not found")
  gene_tsv <- test_path("..", "..", "meta", "features", "homo_sapiens_GRCh38_gene.tsv")
  skip_if_not(file.exists(gene_tsv), "Gene features not found")
  bc_file <- test_path("..", "..", "meta", "features", "human_barcodes_chr19.txt")
  ...
  tryCatch(
    ...
    error = function(e) {
      skip(paste("bam2db zUMIs+gene failed:", conditionMessage(e)))
    }
  )
})
...
test_that("bam2db works with zUMIs protocol + exon features", {
  ...
  human_bam <- test_path("..", "..", "meta", "bam", "human_pbmc_Smart-seq3xpressV3.bam")
  skip_if_not(file.exists(human_bam), "Human BAM not found")
  exon_tsv <- test_path("..", "..", "meta", "features", "homo_sapiens_GRCh38_exon.tsv")
  ...
  tryCatch(
    ...
    error = function(e) {
      skip(paste("bam2db zUMIs+exon failed:", conditionMessage(e)))
    }
  )
})
...
test_that("bam2db works with 10X protocol + gene features", {
  skip_on_cran()
  library(exonBlocks)
  mouse_bam <- test_path("..", "..", "meta", "bam", "mice_brain_10X.bam")
  skip_if_not(file.exists(mouse_bam), "Mouse BAM not found")
  gene_tsv <- test_path("..", "..", "meta", "features", "mus_musculus_GRCm39_gene.tsv")
  skip_if_not(file.exists(gene_tsv), "Mouse gene features not found")
  bc_file <- test_path("..", "..", "meta", "features", "mouse_barcodes_chr19.txt")
  ...
  tryCatch(
    ...
    error = function(e) {
      skip(paste("bam2db 10X+gene failed:", conditionMessage(e)))
    }
  )
})
...
test_that("bam2db works with 10X protocol + exon features", {
  ...
  mouse_bam <- test_path("..", "..", "meta", "bam", "mice_brain_10X.bam")
  skip_if_not(file.exists(mouse_bam), "Mouse BAM not found")
  exon_tsv <- test_path("..", "..", "meta", "features", "mus_musculus_GRCm39_exon.tsv")
  ...
  tryCatch(
    ...
    error = function(e) {
      skip(paste("bam2db 10X+exon failed:", conditionMessage(e)))
    }
  )
})